...
const WAITLIST_COOLDOWN_STORAGE_KEY = 'tether-waitlist-last-submission';
...
function SetWaitlistFeedback(Message, State) {
  ...
  WaitlistFeedback.className = `waitlist-feedback${State ? ` is-${State}` : ''}`;

}

function GetWaitlistCooldownRemainingMs() {

  try {

    const LastSubmissionTimestamp = Number(window.localStorage.getItem(WAITLIST_COOLDOWN_STORAGE_KEY) || 0);
    return Math.max(0, WAITLIST_COOLDOWN_MS - (Date.now() - LastSubmissionTimestamp));
...
function StartWaitlistCooldown() {

  try {

    window.localStorage.setItem(WAITLIST_COOLDOWN_STORAGE_KEY, String(Date.now()));

  } catch (_) {

    // The server-side protections still apply when browser storage is unavailable.

  }

}

if (WaitlistForm && WaitlistSubmitButton) {

  WaitlistForm.addEventListener('submit', async (Event) => {

    Event.preventDefault();

    const HoneypotValue = new FormData(WaitlistForm).get('_gotcha');

    if (typeof HoneypotValue === 'string' && HoneypotValue.trim()) {

      WaitlistForm.reset();
      SetWaitlistFeedback('You are on the list. We will be in touch when early access opens.', 'success');
      return;
    ...
    if (GetWaitlistCooldownRemainingMs() > 0) {

      SetWaitlistFeedback('Thanks. Please wait a minute before trying again.', 'error');
      return;

    }

    const OriginalButtonText = WaitlistSubmitButton.textContent;
    ...
    try {
      ...
      const SubmissionResponse = await fetch(WaitlistForm.action, {
        method: 'POST',
      ...
      WaitlistForm.reset();
      StartWaitlistCooldown();
      SetWaitlistFeedback('You are on the list. We will be in touch when early access opens.', 'success');

    } catch (_) {

      SetWaitlistFeedback('Something went wrong. Please try again in a moment.', 'error');

    } finally {

      WaitlistSubmitButton.disabled = false;
      WaitlistSubmitButton.textContent = OriginalButtonText;
